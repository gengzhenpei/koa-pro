...
const auth = () => {
	return async function (ctx, next) {
		...
		try {
			...
			let whiteUrl = ['/article/v1/login', '/article/v1/user_add', '/article/v1/user_add', '/article/v1/article_list', '/article/v1/category_list']
			//白名单接口不需要验证
			if (whiteUrl.indexOf(url) < 0) {
				...
				if (!user) {
					...
					// ctx.body = extend.resultData(consts.ERROR_CODE.NO_ACCESS_TOKEN)
				}
			}
		} catch (err) {
			if (err.name == 'TokenExpiredError') {
				throw new exception.erroeException(consts.ERROR_CODE.ACCOUNT_CODE_EXPIRED)
				// ctx.body = extend.resultData(consts.ERROR_CODE.ACCOUNT_CODE_EXPIRED)
			} else {
